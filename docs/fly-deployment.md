...
本文档介绍如何在 Fly.io 平台免费部署 antd-gin-admin 系统，支持 GitHub 自动部署。
...
4. **项目代码**：确保包含以下文件：
   ...
   - `backend/configs/config.fly.yaml` - Fly.io 专用配置
...
```bash
# 创建 PostgreSQL 应用
fly postgres create --name antd-gin-postgres --region hkg --vm-size shared-cpu-1x --volume-size 1

# 创建数据库
fly postgres connect -a antd-gin-postgres
# 在 PostgreSQL 中执行：
# CREATE DATABASE antd_gin_admin_scaffold;
# CREATE USER antd-gin WITH PASSWORD 'your_password';
# GRANT ALL PRIVILEGES ON DATABASE antd_gin_admin_scaffold TO antd-gin;
```
...
```bash
# 在项目根目录
cd /path/to/antd-gin-admin

# 初始化 Fly.io 应用（如果还没有）
fly launch --name antd-gin-admin-backend --region hkg

# 设置环境变量
fly secrets set DATABASE_URL="postgresql://antd-gin:password@antd-gin-postgres.internal:5432/antd_gin_admin_scaffold?sslmode=require"
...
- `REACT_APP_API_BASE_URL`: `https://antd-gin-admin-backend.fly.dev`
...
```sql
CREATE DATABASE antd_gin_admin_scaffold;
CREATE USER antd-gin WITH PASSWORD 'your_secure_password';
GRANT ALL PRIVILEGES ON DATABASE antd_gin_admin_scaffold TO antd-gin;
```
...
2. 添加 Secret：`REACT_APP_API_BASE_URL` = `https://antd-gin-admin-backend.fly.dev`
3. 前端会自动使用此环境变量构建
...
| 变量名 | 说明 | 示例 |
|--------|------|------|
| `REACT_APP_API_BASE_URL` | 后端 API 地址 | `https://antd-gin-admin-backend.fly.dev` |
...
- 确认使用 **内部网络地址**（`antd-gin-postgres.internal`）
- 确认 SSL 模式为 `require`
- 检查 PostgreSQL 服务是否运行：`fly status -a antd-gin-postgres`
...
- 查看后端日志：`fly logs -a antd-gin-admin-backend`
